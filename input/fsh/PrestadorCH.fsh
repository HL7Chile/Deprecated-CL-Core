Profile:        PrestadorCH
Parent:         Practitioner
Id:             RecetaPrestadorCh
Title:          "Prestador Core CH"
...
* identifier[RUN].system ^comment = "Se define el el endPoint que apunta a la API que permite validar la existencia del número RUT. Por momento se usará la url = ´http://api_RNPI/RNPI´"
...
* qualification ^slicing.discriminator.path = "code.coding.display"
...
Instance : PrestadorCH
Title : "Ejemplo de Recurso Prestador como base para un Core Nacional"
InstanceOf : RecetaPrestadorCh

 
	* text.div = "<div xmlns='http://www.w3.org/1999/xhtml'>Prestador: Dra. Johanna Rosa de los Angeles Silva Reyes, Chilena de la ciudad de Temuco
				 identificada con un RUN nacional. Ha dejado dos contactos (celular e email). Notifica su direccion	 de domicilio </div>"
...
* identifier[RNPI].system = "http://api_RNPI/RNPI"
...
* qualification[Cert].code.coding.display = "Médico Geneal" //codigo de título profesional Universitario
* qualification[Esp].code.coding.display = "Cardio Vascular"